...
.nt {
  flex: 1; display: block; padding: 0.6rem 0.5rem 0.6rem 1.1rem;
...
.nc-toggle {
  ...
  color: var(--muted); padding: 0 0.85rem; cursor: pointer; font-size: 1.1rem;
  transition: background 0.12s, transform 0.2s, color 0.12s;
  line-height: 1;
}
.nc-toggle:hover { background: var(--blue-light); color: var(--blue); }
.nc.open > .nc-row .nc-toggle { transform: rotate(90deg); color: var(--blue); }
...
/* ── Main ── */
#main { margin-left: var(--w); padding: 2.5rem 4rem 2.5rem 3rem; max-width: 1200px; }
section { padding-bottom: 3rem; margin-bottom: 3rem; border-bottom: 2px solid var(--border); }
...
#totop {
  ...
  /* sit just inside the right edge of the content column */
  right: max(1.5rem, calc(100vw - 1200px - var(--w) + 1.5rem));
...
// ── Chapter expand/collapse — toggle button only, link just navigates ────────
document.querySelectorAll('.nc-toggle').forEach(btn => {
  btn.addEventListener('click', () => {
    const nc = btn.closest('.nc');
